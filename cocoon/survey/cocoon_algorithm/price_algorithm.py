...
class PriceAlgorithm(object):
    ...
    This class is meant to be used as a child class for an algorithm class
      ...
      Attributes:
        ...
        self._desired_price (int): The minimum price the user is willing to spend
    ...
    def populate_price_algorithm_information(self, user_price_scale, user_max_price, user_desired_price,
        ...
        :param user_desired_price:  (int): The min price the user is willing to spend
        :param user_min_price: (int): The minimum price allowed by the user
        """
    ...
    def desired_price(self, new_desired_price):
        ...
        :param new_desired_price: (int): The new min price in dollars
        """
    ...
    def compute_price_score(self, home_price):
        ...
        """
        if home_price < self.min_price or home_price > self.max_price:
            return -1
        elif home_price <= self.desired_price:
            return 1
        else:
            home_price_normalized = home_price - self.desired_price
            home_range = self.max_price - self.desired_price

            if home_range <= 0:
                # If the home_range is zero and the max_price equals the home
                # price then the home is in the range
                if home_price == self.desired_price:
                    return 1
                else:
                    return -1
